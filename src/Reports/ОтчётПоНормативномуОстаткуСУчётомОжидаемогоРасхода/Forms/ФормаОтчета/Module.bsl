...
ТЧ.ГруппаПоЗакупкам = МПЗ.ГруппаПоЗакупкам;	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	ТЧ.ОстатокПоДоговору = ТЧ.ОстатокПоДоговору + ВыборкаДетальныеЗаписи.КоличествоОстаток;
	КонецЦикла;
...
ВыборкаАналогиМПЗ = ТаблицаАналоговМПЗ.НайтиСтроки(Новый Структура("МПЗ",МПЗ));
	Для к = 0 по ВыборкаАналогиМПЗ.Количество()-1 Цикл
		Если (ВыборкаАналогиМПЗ[к].АналогМПЗ.ГруппаПоЗакупкам = Перечисления.ГруппыПоЗакупкам.ПустаяСсылка())или
			 (ВыборкаАналогиМПЗ[к].АналогМПЗ.ГруппаПоЗакупкам = Перечисления.ГруппыПоЗакупкам.а) Тогда	
...
ТекГруппаМПЗ = Неопределено;
	...
	ОблМПЗ.Параметры.Долг = 0;
		...
		КонецЦикла;
	ОблМПЗ.Параметры.ОстатокНеснижаемогоЗапаса = ОблМПЗ.Параметры.КоличествоОстаток-ТЧ.МПЗ.МинОстаток; 
	ОблМПЗ.Параметры.РУСП = ?(ТЧ.МПЗ.СрокПоставки > 0,Окр((ОблМПЗ.Параметры.КоличествоРасход+ОблМПЗ.Параметры.КоличествоСписание+ОблМПЗ.Параметры.КоличествоРеализация)/КолДней*ТЧ.МПЗ.СрокПоставки*7,0,1),0);	
	ОблМПЗ.Параметры.Заказать = (ОблМПЗ.Параметры.КоличествоОстаток+ПолучитьКоличествоОстатокАналогов(ТаблицаАналоговМПЗ,ТаблицаРасходаМПЗ,ТЧ.МПЗ)+ОблМПЗ.Параметры.Долг)-(ТЧ.МПЗ.МинОстаток+ОблМПЗ.Параметры.РУСП);
		Если СУчётомОжидаемогоРасхода Тогда
	    ОблМПЗ.Параметры.Заказать = ОблМПЗ.Параметры.Заказать-ОблМПЗ.Параметры.КоличествоОжидаемыйРасход;
		КонецЕсли;
	ТабДок.Вывести(ОблМПЗ);
		Если СоздатьЗаявкуНаЗакупку Тогда
			Если ОблМПЗ.Параметры.Заказать < 0 Тогда
			Требуется = -1*ОблМПЗ.Параметры.Заказать;
	...
	ВыборкаАналогиМПЗ = ТаблицаАналоговМПЗ.НайтиСтроки(Новый Структура("МПЗ",ТЧ.МПЗ));
		...
		ОблАналог.Параметры.Долг = 0;
			...
			КонецЦикла;
		ОблАналог.Параметры.ОстатокНеснижаемогоЗапаса = ОблАналог.Параметры.КоличествоОстаток-ОблАналог.Параметры.МинОстаток;    
		ОблАналог.Параметры.РУСП = ?(АналогМПЗ.СрокПоставки > 0,Окр((ОблАналог.Параметры.КоличествоРасход+ОблАналог.Параметры.КоличествоСписание+ОблАналог.Параметры.КоличествоРеализация)/КолДней*АналогМПЗ.СрокПоставки*7,0,1),0);	
		ОблАналог.Параметры.Заказать = (ОблАналог.Параметры.КоличествоОстаток+ОблАналог.Параметры.Долг)-(АналогМПЗ.МинОстаток+ОблАналог.Параметры.РУСП);
