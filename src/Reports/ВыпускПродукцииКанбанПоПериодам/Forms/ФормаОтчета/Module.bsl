...
&НаСервере
Функция ПолучитьОснову(Продукция)
НормРасх = ОбщийМодульВызовСервера.ПолучитьНормыРасходовПоВладельцу_Н(Продукция,ТекущаяДата());
...
ТаблицаВыпуска.Колонки.Добавить("Количество",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(7,0)));
...
Запрос = Новый Запрос;
	...
	Запрос.Текст = 
		...
		|	ПланыВыпускаОбороты.Регистратор КАК Регистратор,
		|	ПланыВыпускаОбороты.ПериодДень КАК Период
		|ИЗ
		|	РегистрНакопления.ПланыВыпуска.Обороты(&ДатаНач, &ДатаКон, Авто, Линейка В ИЕРАРХИИ (&СписокЛинеек)) КАК ПланыВыпускаОбороты
		|ГДЕ
		...
		КонецЕсли;
			...
			Запрос.Текст = Запрос.Текст + " И ПланыВыпускаОбороты.Регистратор.ДокументОснование.ЛинияSMD = &ЛинияSMD";		
			КонецЕсли; 
	...
	Запрос.Текст = 
		...
		|	ПланыВыпускаОбороты.КоличествоРасход КАК КоличествоРасход,
		|	ПланыВыпускаОбороты.Номенклатура.Линейка КАК Линейка,
		|	ПланыВыпускаОбороты.Регистратор,
		|	ПланыВыпускаОбороты.ПериодНеделя КАК Период
		|ИЗ
		|	РегистрНакопления.ПланыВыпуска.Обороты(&ДатаНач, &ДатаКон, Авто, Линейка В ИЕРАРХИИ (&СписокЛинеек)) КАК ПланыВыпускаОбороты
		|ГДЕ
		|	ПланыВыпускаОбороты.МаршрутнаяКарта.Ремонт = ЛОЖЬ";
	...
	Запрос.Текст = 
		...
		|	ПланыВыпускаОбороты.Номенклатура.Линейка КАК Линейка,
		|	ПланыВыпускаОбороты.Регистратор,
		|	ПланыВыпускаОбороты.ПериодМесяц КАК Период
		|ИЗ
		|	РегистрНакопления.ПланыВыпуска.Обороты(&ДатаНач, &ДатаКон, Авто, Линейка В ИЕРАРХИИ (&СписокЛинеек)) КАК ПланыВыпускаОбороты
		|ГДЕ
		|	ПланыВыпускаОбороты.МаршрутнаяКарта.Ремонт = ЛОЖЬ";		
...
ТекПродукция = Справочники.Номенклатура.ПустаяСсылка();
		Для каждого ТЧ Из ТаблицаВыпуска Цикл	
			...
			Основа = ПолучитьОснову(ТЧ.Продукция);
			ОблПродукцияОбщие.Параметры.НаименованиеОсновы = СокрЛП(Основа.Наименование);
			ОблПродукцияОбщие.Параметры.Основа = Основа; 
