...
Запрос.Текст = 
	...
	Запрос.Текст = Запрос.Текст + " И Линейки.Ссылка В ИЕРАРХИИ(&СписокЛинеек)";
...
&НаСервере
Процедура РаскрытьНаМПЗ(ПЗ,ЭтапСпецификации,КолУзла)
Запрос = Новый Запрос;
...
ВыборкаНР = РезультатЗапроса.Выбрать();
	...
	БазовоеКоличество = ПолучитьБазовоеКоличество(ВыборкаНР.Норма,ВыборкаНР.Элемент.ОсновнаяЕдиницаИзмерения);
		Если Не ВыборкаНР.Элемент.Канбан.Пустая() Тогда
			Если ВыборкаНР.Элемент.Канбан.Подразделение = ПодразделениеПоставщик Тогда
			ТЧ = ТаблицаИзделий.Добавить();
			ТЧ.МестоХранения = ПЗ.Линейка.МестоХраненияКанбанов;
			ТЧ.Изделие = ВыборкаНР.Элемент;
			ТЧ.Количество = БазовоеКоличество*КолУзла;			
			КонецЕсли;
		КонецЕсли;
	РаскрытьНаМПЗ(ПЗ,ВыборкаНР.Элемент,БазовоеКоличество*КолУзла);
	КонецЦикла;
КонецПроцедуры 
...
Запрос.Текст = 
	...
	Если СписокЛинеек.Количество() > 0 Тогда
	Запрос.Текст = Запрос.Текст + " И МестаХраненияОбороты.Регистратор.ДокументОснование.Линейка В ИЕРАРХИИ(&СписокЛинеек)";
	Запрос.УстановитьПараметр("СписокЛинеек", СписокЛинеек);	
	КонецЕсли;
...
ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ТипЗнч(ВыборкаДетальныеЗаписи.Регистратор) = Тип("ДокументСсылка.ВыпускПродукции")или
			...
			Если Не ВыборкаДетальныеЗаписи.Регистратор.ДокументОснование.ДокументОснование.Ремонт Тогда
			РаскрытьНаМПЗ(ВыборкаДетальныеЗаписи.Регистратор.ДокументОснование,ВыборкаДетальныеЗаписи.Регистратор.ДокументОснование.Изделие,ВыборкаДетальныеЗаписи.Регистратор.ДокументОснование.Количество);
			КонецЕсли; 
		КонецЕсли;
 	КонецЦикла;
...
СебестоимостьОстатокВсего = 0;
	Для каждого ТЧ Из ТЗ Цикл		
		Если ТекМХ <> ТЧ.МестоХранения Тогда
			...
			ОблМестоХраненияИтого.Параметры.КоличествоОстаток = КоличествоОстатокИтого;
 			...
 			ОблМестоХраненияИтого.Параметры.СебестоимостьОстаток = СебестоимостьОстатокИтого;
				Если СебестоимостьРасходИтого > 0 Тогда
				ОблМестоХраненияИтого.Параметры.Коэффициент = СебестоимостьОстатокИтого/СебестоимостьРасходИтого;
				Иначе	
				ОблМестоХраненияИтого.Параметры.Коэффициент = 0;
				КонецЕсли;
		...
		СебестоимостьОстатокИтого = 0;
		КонецЕсли;
 	ОблИзделие.Параметры.Наименование = СокрЛП(ТЧ.Изделие.Наименование);
...
ОблКонец.Параметры.СебестоимостьОстаток = СебестоимостьОстатокВсего;
	Если СебестоимостьРасходВсего > 0 Тогда
	ОблКонец.Параметры.Коэффициент = СебестоимостьОстатокВсего/СебестоимостьРасходВсего;
	Иначе	
	ОблКонец.Параметры.Коэффициент = 0;
	КонецЕсли; 
