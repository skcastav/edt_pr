...
бсдСклад = бсдМенеджерСклады.НайтиПоКоду(Объект.МестоХраненияКод);
	Если Не БухБаза.ЗначениеЗаполнено(бсдСклад) Тогда
	Сообщить("Выберите склад в совместной деятельности.");
	Возврат;	
	КонецЕсли;
...
Запрос.Текст = 
	...
	Запрос.Текст = Запрос.Текст + " И СписаниеМПЗПрочееТабличнаяЧасть.Ссылка.МестоХранения В(&СписокМестХранения)";
	КонецЕсли;
		Если СписокГруппМПЗ.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + " И СписаниеМПЗПрочееТабличнаяЧасть.МПЗ В(&СписокГруппМПЗ)";
		КонецЕсли; 
			Если СписокИсключаемыхСтатей.Количество() > 0 Тогда
			Запрос.Текст = Запрос.Текст + " И НЕ СписаниеМПЗПрочееТабличнаяЧасть.Ссылка.Статья В (&СписокСтатей)";
			КонецЕсли;
...
Запрос.УстановитьПараметр("СписокСтатей", СписокИсключаемыхСтатей);
...
ВыборкаМПЗ = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	ВыборкаДетальныеЗаписи = ВыборкаМПЗ.Выбрать();
		...
		КолРасход = КолРасход + ПолучитьБазовоеКоличество(ВыборкаДетальныеЗаписи.Количество,ВыборкаДетальныеЗаписи.ЕдиницаИзмерения);
		КонецЦикла;
	...
	бсдНовСтр.Количество = КолРасход; 
