...
&НаКлиенте
Процедура ВыгрузитьПослеСбоя(Команда)
	Результат = ВыгрузитьПослеСбояНаСервере();
    ПоказатьПредупреждение(,Результат);
КонецПроцедуры

&НаСервере
Функция ВыгрузитьПослеСбояНаСервере()
	УстановитьПривилегированныйРежим(Истина);
	//вырубаем регламент и фоновое ++
	РегламентныеЗадания.НайтиПредопределенное("ОтложенныйОбменДанными").Использование = Ложь;
	МассивФЗ =  ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("ИмяМетода","СРМ_ОбменВебСервис.ОтложенныйОбменДанными"));
	...
	Узел = СРМ_ОбменВебСервис.ПолучитьМассивУзловПолучателей("ПланОбменаЛинейкаГлавная")[0];
	Выборка = ПланыОбмена.ВыбратьИзменения(Узел,0);
	
	ИмяФайла = Константы.КаталогЛогирования.Получить()+"\ОбменПослеСбоя\Выгрузка.xml";
	...
	Пока Выборка.Следующий() Цикл
		...
		Об = Выборка.Получить();
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Об);
		ПланыОбмена.УдалитьРегистрациюИзменений(Узел,Об);
	КонецЦикла;
	...
	Попытка
		Результат = Прокси.ОбменПослеСбоя(Новый ХранилищеЗначения(Новый Структура("ИмяФайла",ИмяФайла), Новый СжатиеДанных(9)));	
	Исключение
	...
	Сообщить("Ошибки:");
	Для каждого Ошибка из СтруктураРезультата.Ошибки цикл
		Сообщить(Ошибка.ИмяОбъекта+"("+Строка(Ошибка.Хеш)+") - "+Ошибка.Ошибка);
	КонецЦикла;	
	
	Возврат "Выгрузка завершена";
	РегламентныеЗадания.НайтиПредопределенное("ОтложенныйОбменДанными").Использование = Истина;
