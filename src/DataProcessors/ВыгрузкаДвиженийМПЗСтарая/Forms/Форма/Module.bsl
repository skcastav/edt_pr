...
Запрос.Текст = 
	...
	|	И ДвижениеМПЗТабличнаяЧасть.Ссылка.МестоХраненияВ В(&МестоХраненияВ)
...
Запрос.УстановитьПараметр("МестоХранения", МестоХраненияОткуда);
Запрос.УстановитьПараметр("МестоХраненияВ", СписокМестХраненияКуда);
РезультатЗапроса = Запрос.Выполнить();
ВыборкаМПЗ = РезультатЗапроса.Выбрать();
	Пока ВыборкаМПЗ.Следующий() Цикл
	ТЧ = ТаблицаМПЗ.Добавить();
	ТЧ.МПЗ = ВыборкаМПЗ.МПЗ;
	ТЧ.Количество = ПолучитьБазовоеКоличество(ВыборкаМПЗ.Количество,ВыборкаМПЗ.ЕдиницаИзмерения);
	КонецЦикла;
ТаблицаМПЗ.Свернуть("МПЗ","Количество");
ТаблицаМПЗ.Сортировать("МПЗ");
	Для каждого ТЧ Из ТаблицаМПЗ Цикл
	бсдНоменклатура = НайтиСоздатьНоменклатуру(БухБаза, ТЧ.МПЗ, бсдГрНомПоУмолчанию);	
		Если Не БухБаза.ЗначениеЗаполнено(бсдНоменклатура) Тогда
		Сообщить("Не найден МПЗ "+ТЧ.МПЗ);
		Продолжить;	                                     
		КонецЕсли;		
	...
	бсдНовСтр.Количество = ТЧ.Количество; 
