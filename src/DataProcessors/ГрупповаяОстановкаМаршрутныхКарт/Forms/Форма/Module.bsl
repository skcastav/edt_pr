...
Запрос = Новый Запрос;
	...
	Для каждого Маска Из СписокМасокТоваров Цикл
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	МаршрутнаяКарта.Статус В(&Статусы)
		|	И МаршрутнаяКарта.ДокументОснование.Контрагент В(&СписокКонтрагентов)
		|	И МаршрутнаяКарта.Номенклатура.Товар.Наименование ПОДОБНО &Наименование
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерОчереди,
		|	МаршрутнаяКарта.Номер";
	Запрос.УстановитьПараметр("СписокКонтрагентов",СписокКонтрагентов);
	Запрос.УстановитьПараметр("Наименование",СокрЛП(Маска.Значение)+"%");
...
Настройки = Новый Структура("СписокКонтрагентов,СписокМасокТоваров",СписокКонтрагентов,СписокМасокТоваров);
Возврат(Новый Структура("КлючНазначенияИспользования,Настройки","ГрупповаяОстановкаМаршрутныхКарт",Настройки));
КонецФункции
...
Результат = ОткрытьФормуМодально("ОбщаяФорма.НастройкиФорм",НастройкиНаСервере());
	...
	СписокКонтрагентов = Результат.СписокКонтрагентов;
	СписокМасокТоваров = Результат.СписокМасокТоваров;	
	КонецЕсли;
КонецПроцедуры
