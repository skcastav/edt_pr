...
Запрос.Текст = 
	...
	|ГДЕ
	|	ПередачаНаЛинейку.Проведен = ИСТИНА
	|	И ПередачаНаЛинейку.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ПередачаНаЛинейку.МестоХранения = &МестоХранения
	|	И ПередачаНаЛинейку.МестоХраненияКанбанов В(&МестоХраненияКанбанов)
	|ИТОГИ
	|	СУММА(Количество)
	|ПО
	|	МПЗ";
...
Запрос.УстановитьПараметр("МестоХраненияКанбанов", СписокМестХраненияКуда);
...
ВыборкаМПЗ = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	бсдНовСтр.Количество = ВыборкаМПЗ.Количество; 
