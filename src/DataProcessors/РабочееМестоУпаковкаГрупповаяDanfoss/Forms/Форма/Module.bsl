...
Запрос.Текст = 
	...
	|	И ЭтапыПроизводственныхЗаданийСрезПоследних.ПЗ.ДокументОснование.Статус <> 2
	|	И ЭтапыПроизводственныхЗаданийСрезПоследних.ПЗ.ДатаЗапуска <> ДАТАВРЕМЯ(1,1,1,0,0,0)
	|	И ЭтапыПроизводственныхЗаданийСрезПоследних.ПЗ.ДатаГрупповойУпаковки = ДАТАВРЕМЯ(1,1,1,0,0,0)
	|	И ЭтапыПроизводственныхЗаданийСрезПоследних.Ремонт = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаНачала";
...
ВыборкаДетальныеЗаписи = Результат.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл	
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДатаНачала) Тогда
			Если Найти(ВыборкаДетальныеЗаписи.ПЗ.ВозвратнаяТара,Префикс) = 0 Тогда
			Продолжить;
			КонецЕсли;		
		КонецЕсли; 
	ТЧ = Объект.Упаковка.Добавить(); 
	...
	ТЧ.Тара = ВыборкаДетальныеЗаписи.ПЗ.ВозвратнаяТара;
	КонецЦикла; 
...
&НаСервере
Функция ПолучитьНумератор(Товар)
Запрос = Новый Запрос;

Запрос.Текст = 
	"ВЫБРАТЬ
	|	НумераторГрупповыхУпаковокDanfoss.Ссылка КАК Ссылка
	|ИЗ
	...
	|	И НумераторГрупповыхУпаковокDanfoss.Префикс = &Префикс";
	...
	КонецЕсли;
		...
		КонецЕсли; 
			Если Изделие.Товар.ТребуетсяВводНапряженияБатарейки Тогда
				...
				КонецЕсли;
			КонецЕсли;
НГУ = ПолучитьНумератор(Изделие.Товар);
	Если НГУ = Неопределено Тогда
	Сообщить("Нумератор групповой упаковки с выбранным префиксом не найден!");
	Возврат(Ложь);		
	КонецЕсли;
КолУпаковано = 0;
	Для каждого ТЧ_УП Из Объект.Упаковка Цикл
		Если ТЧ_УП.Изделие = Изделие Тогда
			Если Найти(ТЧ_УП.Тара,Префикс) > 0 Тогда
			КолУпаковано = КолУпаковано + 1;
			КонецЕсли;					
		КонецЕсли;   	
	КонецЦикла; 	
		Если КолУпаковано < НГУ.КоличествоВТаре Тогда
		Тара = НГУ.Префикс+НГУ.СледующийНомер;
		Иначе
...
Выборка = Объект.Упаковка.НайтиСтроки(Отбор);
	Если Выборка.Количество() > 0 Тогда	
		Если Не МТКОстановлена(Выборка[0].ПроизводственноеЗадание) Тогда
		...
		Элементы.Упаковка.ТекущаяСтрока = Стр;
			...
			КонецЕсли;
		Иначе
		Сообщить("МТК по данному изделию остановлена!");
		КонецЕсли; 
	Иначе
...
ТаблицаГрупповойУпаковки.Очистить();
	...
	НГУ = ПолучитьНумератор(ТЧ.Изделие.Товар);
		Если НГУ = Неопределено Тогда
		Продолжить;		
		КонецЕсли; 	
	Выборка = ТаблицаГрупповойУпаковки.НайтиСтроки(Новый Структура("Нумератор",НГУ));
		Если Выборка.Количество() = 0 Тогда
		ТЧ_ТГУ = ТаблицаГрупповойУпаковки.Добавить();
		ТЧ_ТГУ.Нумератор = НГУ;
    	ТЧ_ТГУ.Товар = ТЧ.Изделие.Товар;
			Если ЗначениеЗаполнено(ТЧ.Поступление) Тогда
			ТЧ_ТГУ.Тара = ТЧ.Тара;
			ТЧ_ТГУ.Количество = 1;
			КонецЕсли; 
...
Парам = Новый Структура("Результат, Товар, Количество,QRCode",Ложь,Справочники.Товары.ПустаяСсылка(),0,"");
...
ТД.Вывести(ОблШапка);
	...
	КолВсего = 0;
		Для каждого ТЧ Из Объект.Упаковка Цикл
			...
			Продолжить;	
			КонецЕсли;
				Если ТЧ.Изделие.Товар <> Товар Тогда
				Продолжить;
				КонецЕсли;
		НомСтр = НомСтр + 1;
	...
	НГУОбъект = НГУ.ПолучитьОбъект();
		Если НГУ.СледующийНомер >= 101 Тогда		
		НГУОбъект.СледующийНомер = 1;		
		Иначе
...
&НаСервере
Функция ПечатьЭтикеткиГрупповойУпаковкиНаСервере(ТД_ГУ,Товар,Количество,QRCode)
ШаблонПД = Товар.ТоварнаяГруппа.ШаблоныПечатныхДокументов;
	Для каждого ТЧ Из ШаблонПД Цикл
		...
		ТекНаимен = СокрЛП(Товар.Наименование);
		Модиф = "";
		...
		ОблЭтикетка.Параметры.ПолнНаименование = СокрЛП(Товар.ПолнНаименование);
		ОблЭтикетка.Параметры.КодDanfoss = Товар.КодДанфосс;
...
Парам = ПечатьГрупповойУпаковкиНаСервере(ТД);
	...
	ТД.Показать("Отчёт по упаковочной таре. Выпуск на "+Формат(ДатаВыпуска,"ДФ=dd.MM.yyyy"));
		Если Не Парам.Товар.Пустая() Тогда
		ТД_ГУ = Новый ТабличныйДокумент;

			Если ПечатьЭтикеткиГрупповойУпаковкиНаСервере(ТД_ГУ,Парам.Товар,Парам.Количество,Парам.QRCode) Тогда
...
&НаСервере
Функция ПовторнаяПечатьГрупповойУпаковкиНаСервере(ТД,ВыбДата,ВыбТара,ВыбТовар)
Парам = Новый Структура("Результат, Товар, Количество,QRCode",Ложь,ВыбТовар,0,""); 
ТаблицаВыпуска = Новый ТаблицаЗначений;
...
ТД.Вывести(ОблШапка);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПроизводственноеЗадание.Изделие.Товар = &Товар
...
Запрос.Текст = 
	...
	|	Документ.ПроизводственноеЗадание КАК ПроизводственноеЗадание
	|ГДЕ
	|	ПроизводственноеЗадание.ВозвратнаяТара = &ВозвратнаяТара
	|	И ПроизводственноеЗадание.ДатаГрупповойУпаковки МЕЖДУ &ДатаНач И &ДатаКон";
Запрос.УстановитьПараметр("ВозвратнаяТара", ВыбТара);
Запрос.УстановитьПараметр("ДатаНач", НачалоДня(ВыбДата));
Запрос.УстановитьПараметр("ДатаКон", КонецДня(ВыбДата));
...
ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если СписокИзделий.НайтиПоЗначению(ВыборкаДетальныеЗаписи.Товар) = Неопределено Тогда
		СписокИзделий.Добавить(ВыборкаДетальныеЗаписи.Товар);
		КонецЕсли;
	КонецЦикла;
...
ВыбДата = ТекущаяДата();
	...
	КонецЕсли;
ВыбТара = 0;
	Если Не ВвестиСтроку(ВыбТара,"Введите номер тары",4,Ложь) Тогда
	Возврат;
	КонецЕсли;
ВыбИзделие = ПолучитьПустуюСсылкуНоменклатуры();
СписокИзделий = ПолучитьСписокИзделийВГрупповыхУпаковках(ВыбДата,ВыбТара);
...
Парам = ПовторнаяПечатьГрупповойУпаковкиНаСервере(ТД,ВыбДата,ВыбТара,ВыбИзделие);
	Если Парам.Результат Тогда
