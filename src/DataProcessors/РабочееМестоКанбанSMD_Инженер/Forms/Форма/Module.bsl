...
&НаСервере
Функция НесоответствиеВФайлах(МТК)
ТЧ = МТК.Остановки[МТК.Остановки.Количество()-1];
	Если СокрЛП(ТЧ.Причина.Наименование) = "SMD. Несоответствие файла станка и СП" Тогда
	Возврат(Истина);
	Иначе
	Возврат(Ложь);
	КонецЕсли; 
...
ВыборкаДетальныеЗаписи = Результат.Выбрать();
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если НесоответствиеВФайлах(ВыборкаДетальныеЗаписи.ПЗ.ДокументОснование) Тогда
...
ТаблицаАналогов = ОбщегоНазначенияПовтИсп.ПолучитьАналогиНормРасходов(НР,ТекущаяДата(),Истина);
...
Состояние("Обработка файлов программ...",,"Проверка исходных файлов");
	...
	ОбработатьТаблицуФайлов();
		...
		Сообщить("Имеются частичные замены на аналоги! Внесите соответствующие изменения в файлы!");
...
Процедура Завершить(Команда)
	Если Не Объект.ПроизводственноеЗадание.Пустая() Тогда
		Если ПроверитьФайлыСАналогами() = 0 Тогда
			Если ЗавершитьНаСервере() Тогда
			ОчиститьСсылкуНаПЗ();
			КонецЕсли;
		Иначе
		Сообщить("В файлах для станка обнаружены ошибки (см. таблицу сверки)!");
		КонецЕсли; 
	Иначе
...
&НаКлиенте
Процедура ОткрытьИсходныйФайлДляСтанка(Команда)
ЗапуститьПриложение(Элементы.ТаблицаФайлов.ТекущиеДанные.ИсходныйФайл);
КонецПроцедуры
