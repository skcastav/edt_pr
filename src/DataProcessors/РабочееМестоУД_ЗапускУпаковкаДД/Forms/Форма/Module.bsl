...
&НаСервере
Функция НеобходимоВвестиВозвратнуюТару()
	Если СписокРабочихМест[0].Значение.Код = 1 Тогда
		Если СписокРабочихМест[0].Значение.ВозвратнаяТара Тогда
		Возврат(Истина);
		КонецЕсли; 
	КонецЕсли;
Возврат(Ложь); 
...
ОблБирка.Параметры.Номер = Объект.ПроизводственноеЗадание.БарКод;
...
&НаСервере
Функция ПечатьРемонтнойБирки(ПричиныРемонта)
ТабДок = Новый ТабличныйДокумент;
...
ОблБирка.Параметры.ВозвратнаяТара = Объект.ПроизводственноеЗадание.ВозвратнаяТара;
ОблБирка.Параметры.ПричинаРемонта = ПричиныРемонта.ПричинаРемонта;
ОблБирка.Параметры.Комментарий = ПричиныРемонта.Комментарий;
ТабДок.Вывести(ОблБирка);
Возврат(ТабДок);
КонецФункции 

&НаКлиенте
Функция ВводВозвратнойТары(Команда,ДанныеСканера = "")
	Если НеобходимоВвестиВозвратнуюТару() Тогда
	НомерВТ = ДанныеСканера;
		Если Не ЗначениеЗаполнено(НомерВТ) Тогда
			...
			КонецЕсли;				
		КонецЕсли;				
			Если ОбщийМодульВызовСервера.СохранитьНомерВозвратнойТары(Объект.ПроизводственноеЗадание,НомерВТ) Тогда
			ТД = ПечатьРабочейБирки();
			ТД.Показать("Рабочая бирка");						
			Иначе
			Сообщить("Номер возвратной тары не сохранён!");
			ОчиститьСсылкуНаПЗ();
			Возврат(Ложь);
			КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПолучитьЗадание(Команда,ДанныеСканера = "")
...
Результат = ПолучитьНезавершенноеЗадание();
	Если Результат = 0 Тогда
		Если ПолучениеЗаданияСканированием() Тогда
			Если Не ЗначениеЗаполнено(ДанныеСканера) Тогда
			ВвестиСтроку(ДанныеСканера,"Введите бар-код",18);
			КонецЕсли;
		Результат = ПолучитьЗаданиеНаПромежуточномЭтапеНаСервере(ДанныеСканера);
			Если Результат = 0 Тогда
		...
		ИначеЕсли ПолучениеЗаданияСканированиемВТ() Тогда
			Если Не ЗначениеЗаполнено(ДанныеСканера) Тогда
			ВвестиСтроку(ДанныеСканера,"Введите номер возвратной тары",4);
			КонецЕсли;
		Результат = ПолучитьЗаданиеПоВозвратнойТаре(ДанныеСканера);		  	
		Иначе
	...
	КонецЕсли;
		...
		Элементы.Ремонт.Доступность = Истина;
			Если НеобходимоВвестиВозвратнуюТару() Тогда	
			ПоказатьОповещениеПользователя("ВНИМАНИЕ!",,"Необходимо ввести возвратную тару!",БиблиотекаКартинок.Пользователь);
			КонецЕсли;
		Элементы.СоздатьПодчиненнуюМТК.Доступность = НетПодчненныхМТК();  		
...
Функция ЗавершитьЗаданиеНаСервере()
	Если (НеобходимоВвестиВозвратнуюТару())и(Не ЗначениеЗаполнено(НомерВТ)) Тогда
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Массив = ОбщийМодульВызовСервера.РазложитьСтрокуВМассив(Данные,";");
		...
		Иначе		
			Если ПолучениеЗаданияСканированием() или ПолучениеЗаданияСканированиемВТ() Тогда
			ПолучитьЗадание(Неопределено,Данные);
			Иначе
				Если Не Объект.ПроизводственноеЗадание.Пустая() Тогда
					Если НеобходимоВвестиВозвратнуюТару() Тогда			
					ВводВозвратнойТары(Неопределено,Данные);
				    КонецЕсли;
				КонецЕсли; 
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ПолучитьНомерРемонтнойКарты()
Запрос = Новый Запрос;
...
&НаСервере
Процедура СоздатьПодчиненнуюМТКНаСервере()
	Попытка
	НачатьТранзакцию();ПараметрыСеанса.АктивнаТранзакция = ПараметрыСеанса.АктивнаТранзакция + 1;	
		Если Не РаскрытьНаМПЗ(Объект.ПроизводственноеЗадание.ДокументОснование,Объект.ПроизводственноеЗадание.Изделие,Объект.ПроизводственноеЗадание.ДокументОснование.Количество,Истина) Тогда
		Сообщить(Объект.ПроизводственноеЗадание.ДокументОснование.Номер+" - подчиненные МТК не созданы!");
		ОтменитьТранзакцию();ПараметрыСеанса.АктивнаТранзакция = ПараметрыСеанса.АктивнаТранзакция-1;ПараметрыСеанса.ОбъектыСозданныеВТранзакции = Новый ХранилищеЗначения(Новый Массив);
		КонецЕсли;
	ЗафиксироватьТранзакцию();ПараметрыСеанса.АктивнаТранзакция = ПараметрыСеанса.АктивнаТранзакция-1;Если ПараметрыСеанса.АктивнаТранзакция = 0 тогда СРМ_ОбменВебСервис.ОтправкаПослеТранзакции();КонецЕсли;
	Исключение
...
&НаКлиенте
Процедура СоздатьПодчиненнуюМТК(Команда)
СоздатьПодчиненнуюМТКНаСервере();
КонецПроцедуры
