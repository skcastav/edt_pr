...
Запрос.Текст =
	 ...
	 |ГДЕ
	 |	ТИПЗНАЧЕНИЯ(МестаХраненияОстатки.МПЗ) = ТИП(Справочник.Материалы)
	 |	И МестаХраненияОстатки.МестоХранения В ИЕРАРХИИ(&СписокМестХранения)
	 |ИТОГИ
	 |	СУММА(Количество)
	 |ПО
	 |	МестоХранения,
...
ВыборкаМХ = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Склад	= БухБаза.Справочники.Склады.НайтиПоКоду(СокрЛП(ВыборкаМХ.МестоХранения.КодВБухБазе));
		...
		бсНовДок.Комментарий = "Выгрузка из производственной базы за "+Формат(НаДату,"ДФ=dd.MM.yyyy");
		Иначе
	...
	ВыборкаМПЗ = ВыборкаМХ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		...
		ТЧ_П.Цена = ОбщийМодульВызовСервера.ПолучитьПоследнююЦену(ВыборкаМПЗ.МПЗ,НаДату);
