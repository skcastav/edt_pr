...
ПЗ = ТЧ.ПЗ;
	...
	СвободныйВыбор = Не СвободныйВыбор;
...
Функция ЗавершитьНаСервере(Количество)
	...
	Парам = Новый Структура("РабочееМесто,Количество",ОбщийМодульВызовСервера.ПолучитьСледующееРабочееМесто(Объект.РабочееМесто),Количество);
...
Функция ЗавершитьИРазделитьНаСервере(Количество,ПричинаОстановки)
	...
	Парам = Новый Структура("РабочееМесто,Количество",ОбщийМодульВызовСервера.ПолучитьСледующееРабочееМесто(Объект.РабочееМесто),Количество - КолБрак);
	ОбщийМодульРаботаСРегистрами.СоздатьЭтапПроизводственногоЗаданияКанбан(Объект.ПроизводственноеЗадание,Парам);			
	...
	ПричинаОстановки = "";
		Если ВвестиСтроку(ПричинаОстановки,"Введите причину остановки",0,Истина) Тогда
			...
			ПричинаОстановки = СокрЛП(ПричинаОстановки)+" ("+Объект.Исполнитель+")";
				...
				ИначеЕсли КоличествоВыпущено < МаксКоличество Тогда
					Если СписокЭтапов[СписокЭтапов.Количество()-1].Значение = ЭтапПроизводства Тогда
						Если ЗавершитьИРазделитьНаСервере(КоличествоВыпущено,ПричинаОстановки) Тогда	
						ОчиститьСсылкуНаПЗ();
						ПолучитьДанныеПоЗаданиямНаСервере();
						КонецЕсли;
					Иначе
