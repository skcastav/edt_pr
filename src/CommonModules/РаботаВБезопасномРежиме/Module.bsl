...
Процедура ПроверитьИмяМетодаКонфигурации(Знач ИмяМетода) Экспорт
	...
	ЧастиИмени = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяМетода, ".");
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось получить менеджер для объекта ""%1""'"), Имя);
...
Функция ВозможноВыполнениеОбработчиковУстановкиПараметровСеансаБезУстановкиБезопасногоРежима() Экспорт
	...
	Исключение
		...
		ТекстЗаписиЖР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаписиЖР, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
